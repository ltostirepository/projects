...
void initpeaks( Peaks *p ){ p->N=0; p->t.clear(); p->V.clear(); p->dt.clear(); p->xt.clear(); p->Q.clear(); }
void ScaleGraph( TGraph* g, Double_t scale){
    for(int ip=0; ip<g->GetN(); ip++){ g->SetPoint(ip,g->GetX()[ip],scale*g->GetY()[ip]); }
}
...
void FindPeaks( TGraph* g, Peaks *peaks, Double_t threshold, bool average, bool fixleftside ){
    ...
    while( (ip+1)<g->GetN() )
    {
        ...
        else
        {
            ...
            Int_t localxmax=-999;
            for( Int_t jj=minip; jj<maxip; jj++ ){ if( fabs(Y[jj])>localVmax ) { localVmax=Y[jj]; localtmax=X[jj]; localxmax=jj; } }
            if(average)
            {
                int nn=1;
...
void FindNextPeaks( TGraph *g, Peaks* peaksdled, Peaks *peaks, Double_t twindow, bool average ){
    ...
    for(int ip=0; ip<peaksdled->N; ip++)
    {
        ...
        {
            if( Y[ix] >localVmax && found[ix]==false) { localVmax=Y[ix]; localtmax=X[ix]; localxmax=ix; }
...
void SetHTime(TH1* h){
    h->GetXaxis()->SetTimeFormat("%H:%m%F1970-01-01 00:00:00"); h->GetXaxis()->SetTimeDisplay(true); return;
}
